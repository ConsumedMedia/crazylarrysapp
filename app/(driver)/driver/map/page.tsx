...
export default async function DriverMapPage() {
  ...
  return (
    <div className="flex flex-col gap-4 lg:grid lg:grid-cols-[1fr_360px] lg:items-start lg:gap-5">
      {/* Map + next stop — full width on phone, left column on tablet. */}
      <div className="flex flex-col gap-4">
        ...
        {open.length === 0 ? (
          ...
          <div className="-mx-4 h-[260px] border-b-2 border-line-strong lg:mx-0 lg:h-[520px] lg:border-2">
            <iframe
              title="Today's route"
              width="100%"
              height="100%"
          ...
          <div className="flex h-[220px] flex-col items-center justify-center gap-1 border-2 border-dashed border-line-strong bg-surface-2 p-4 text-center lg:h-[520px]">
        ...
        {next && (
          <div className="border-2 border-teal bg-surface p-3.5">
            ...
            <a
              ...
              className="cl-btn mt-3 flex w-full items-center justify-between bg-ink px-4 py-3.5 text-[14px] font-extrabold text-surface"
            >
      ...
      {/* Rest of the run — stacks below on phone, rail on the right on tablet. */}
      {rest.length > 0 && (
        <div className="border-2 border-line-strong bg-surface lg:self-start">
          ...
          {rest.map((j, i) => (
            <Link
              key={j.id}
              href={`/driver/${j.id}`}
              className="cl-btn flex items-center gap-3 border-b border-line px-3.5 py-3 last:border-b-0"
            >
